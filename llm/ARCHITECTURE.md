...
`ExtensionContext.withinTransaction<T>(fn: () => T): T` は複数の変更を 1 つの undo ステップへ束ねる。SDK の型定義はコールバックが同期であること（内部で `await` しないこと）を要求し、ネストしたトランザクションは最外へ collapse される。

このため、**生成と生成後の属性設定を 1 undo ステップへ束ねることはできない**。生成（`createMidiTrack` / `createScene` / `createMidiClip` など）は非同期で、結果を受け取る時点でトランザクションは閉じている。本リポジトリでは次の形を取る。

1. 生成の発行を 1 トランザクションで行い、Promise を返す（コールバック自体は同期）
2. 生成結果に対する属性設定を、まとめて別の 1 トランザクションで行う

結果として Live の undo 履歴には「生成」と「属性設定」の 2 ステップが残る。属性ごとの setter は自前でトランザクションを張るため、まとめないと属性の数だけステップが増える。

テストのフェイク（`apps/extension/src/test-support/fake-transaction.ts`）は SDK と同じ同期シグネチャを持ち、`async` コールバックが渡された場合に失敗させて契約違反を検出する。

SDK が示す回避策は `Promise.all([...])` を返して複数の非同期操作を 1 トランザクションで発行する形であり、生成結果へ書き戻す用途には適用できない。
