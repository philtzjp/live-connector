...
9. MUST: JSON 直列化は `packages/json` に集約する; NEVER: 応答・ログ・永続化の出力で `JSON.stringify` を直接呼ばない（`bigint` で `TypeError` になるため）
10. MUST: すべての型を専用ディレクトリ内のファイルで定義する
11. SHOULD: 変数名をオブジェクト化して単一ワードに正規化する（例: `worksName` → `works.name`）
12. MUST: モジュラーモノリスアーキテクチャを採用する
13. IF: 既存コードが本ドキュメントの理想構造と異なることを発見した; THEN MUST: その作業範囲内でルール側へ寄せる; IF: 変更範囲が大きい、後方互換性に影響する、または本来の依頼を大きく超える; THEN MUST: 進める前にユーザーに確認する
