...
6. MUST: マージ後、対象 Issue を手動でクローズする（英語の自動クローズ記法を使わないため。`gh issue close <番号>`）; IF: PR 作成時に GitHub の Development で Issue をリンクした; THEN MUST: 自動クローズされているか確認する; IF: マージ・レビュー・検証を保留する、または解決が pending; THEN MAY: 対象 Issue をクローズせず開いたまま「保留（pending review）」として状況・再開条件をコメントに記録する（同一事象での Issue 重複起票を防ぐため）; NEVER: 未解決の同一事象について Issue を重複起票しない
7. IF: 同一事象について複数の Issue が起票・クローズされていた; THEN MUST: 最古の Issue を親（正本）として sub-issue 関係を付け、後発を duplicate としてクローズし、変更内容を備考として各 Issue にコメントする
8. SHOULD: 作業ブランチの削除は、リモート状態とユーザー意図を確認してから行う
