---
name: api-design
description: API エンドポイントの設計・実装・変更を行うとき（HTTP ハンドラ、OpenAPI スキーマ、ヘルスチェック、ルーティング、認証方式の追加、MCP サーバーの構築など）に参照する。OpenAPI 準拠、RFC 9457 エラー、パスバージョニング、Bearer 認証（loopback サービスは Origin 検証で代替可）、Spectral リント、HTTP サーバー層の統一と MCP トランスポートの選定を定義する。
---
...
6. MUST: ネットワークに公開される API は Bearer 認証を使用する; IF: loopback（localhost）にのみバインドするローカルサービス; THEN MAY: Bearer 認証に代えて Origin / Host ヘッダ検証で保護する（DNS リバインディング対策）
